...
const Hotels = () => (
  <div className="pt-32 pb-24 max-w-7xl mx-auto px-6 sm:px-8">
    <div className="text-center mb-16 md:mb-20">
      <div className="inline-block px-4 py-1.5 bg-indigo-50 text-indigo-600 rounded-full text-[10px] font-black uppercase tracking-widest mb-4">Our Packages</div>
      <h1 className="text-4xl md:text-6xl font-black text-slate-900 mb-6 tracking-tighter">แพ็คเกจเที่ยวทั้งหมด</h1>
      <p className="text-slate-500 font-medium text-lg max-w-xl mx-auto">ดิวพิเศษรถเช่าพร้อมที่พักพรีเมียมที่คุณหาไม่ได้จากที่ไหน</p>
    </div>
      ...
      {MOCK_HOTELS.map((item) => (
        <Link to={`/hotels/${item.id}`} key={item.id} className="group flex flex-col bg-white rounded-3xl overflow-hidden shadow-sm hover:shadow-2xl transition-all duration-500 border border-slate-100">
          ...
          <div className="p-8 flex flex-col flex-1">
            <h3 className="font-black text-2xl text-slate-900 mb-4 leading-tight">{item.name}</h3>
            <p className="text-slate-400 text-sm mb-10 flex items-center gap-2 font-bold"><MapPin size={18} className="text-indigo-600" /> {item.location}</p>
            <div className="mt-auto flex justify-between items-center pt-8 border-t border-slate-50">
              <div>
                ...
                <p className="font-black text-3xl text-slate-900">฿{item.price_per_night.toLocaleString()}</p>
              </div>
