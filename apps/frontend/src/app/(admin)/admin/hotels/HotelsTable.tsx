...
export default function HotelsTable({ hotels }: HotelsTableProps) {
  ...
  const filtered = hotels.filter((hotel) => {
    const matchesSearch =
      !search ||
      hotel.name_th.toLowerCase().includes(search.toLowerCase()) ||
      hotel.name_en.toLowerCase().includes(search.toLowerCase()) ||
      (hotel.location_th || '').toLowerCase().includes(search.toLowerCase()) ||
      (hotel.location_en || '').toLowerCase().includes(search.toLowerCase()) ||
  ...
  const paginatedHotels = filtered.slice(
              ...
              {paginatedHotels.length === 0 && (
                <tr>
                  <td colSpan={5} className="px-6 py-8 text-center text-slate-500">
                    {search || filterStatus !== 'ALL' ? 'ไม่พบผลลัพธ์' : 'ยังไม่มีโรงแรม'}
                  </td>
                </tr>
              )}
            </tbody>
